...
CompanyRef.create = async function (newCompRef, result) {
  const checkCompanyRef = await pool.query(
    `SELECT * FROM user_has_company WHERE id_company = ${newCompRef.id_company} AND id_user = ${newCompRef.id_user}`,
  )
...
CompanyRef.findCompaniesByUserId = function (id, result) {
  pool.query(
    'SELECT * FROM user_has_company INNER JOIN company ON user_has_company.id_company = company.id WHERE id_user = ? ',
    id,
    (err, res) => {
      if (err) {
        result(err, null)
...
CompanyRef.findById = function (newCompRef, result) {
  pool.query(
    'SELECT * FROM user_has_company INNER JOIN user ON user_has_company.id_user = user.id INNER JOIN company ON user_has_company.id_company = company.id WHERE user_has_company.id = ?',
    newCompRef,
    (err, res) => {
      if (err) {
        result(err, null)
...
CompanyRef.findUsersByCompanyId = function (id, result) {
  pool.query(
    'SELECT * FROM user_has_company INNER JOIN user ON user_has_company.id_user = user.id WHERE id_company = ?',
    id,
    (err, res) => {
      if (err) {
        result(err, null)
...
CompanyRef.update = function (id, newCompRef, result) {
  pool.query(
    'UPDATE user_has_company SET id_user=?, id_company=?, id_state=? WHERE id =' +
      id,
...
CompanyRef.delete = function (id, result) {
  pool.query(
    'UPDATE user_has_company SET state = 2 WHERE id = ?',
    [id],
    (err, res) => {
      if (err) {
        result(err, null)
...
CompanyRef.endpoint = function (name, token, result) {
